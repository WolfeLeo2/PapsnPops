...
Future<void> _runLocalCompatMigrations() async {
  ...
  try {
    await db.execute(
      'ALTER TABLE item_groups ADD COLUMN default_pricing_unit TEXT DEFAULT "piece"',
    );
  } catch (_) {}

  try {
    await db.execute(
      'ALTER TABLE item_groups ADD COLUMN default_coverage_per_box REAL DEFAULT 1.0',
    );
  } catch (_) {}
