...
class _EditInvoiceScreenState extends ConsumerState<EditInvoiceScreen> {
  ...
  double _computeSubtotal() {
    ...
    for (final item in _items) {
      final enteredPrice =
          double.tryParse(item.priceCtr.text) ??
          (item.isSqmBased
              ? (item.initialPrice / item.coveragePerBox)
              : item.initialPrice);
      final enteredQty =
          double.tryParse(item.qtyCtr.text) ??
          (item.isSqmBased
              ? (item.initialQty * item.coveragePerBox)
              : item.initialQty.toDouble());
